...
function updateStats() {
    ...
    for (let i = 0; i < rows.length; i++) {
        ...
        if (stat.includes("SOLVED!")) {
            winCt++;
        } else if (stat.includes("MAXED")) {
            loseCt++;
        } else {
            emptyCt++;
        }
        ...
        if (btn.value && btn.value.length > 0) {
            ...
            if (stat.includes("SOLVED!")) {
                iterSolCt.push(parseInt(iter.split("/")[0]));
                timeSolCt.push(parseFloat(time.replace('s', '')));
                solSolCt.push(btn.value.length);
            }
        }
    }
    ...
    let iterSolCtAvg = (iterSolCt.length > 0 ? iterSolCt.reduce((a, b) => a + b) / iterSolCt.length : 0);
    let timeSolCtAvg = (timeSolCt.length > 0 ? timeSolCt.reduce((a, b) => a + b) / timeSolCt.length : 0);
    let solSolCtAvg = (solSolCt.length > 0 ? solSolCt.reduce((a, b) => a + b) / solSolCt.length : 0);
    let levAcc = ((winCt + loseCt) > 0 ? winCt / (winCt + loseCt) : 0);

    console.log('Avg. # iterations: ' + Math.round(iterSolCtAvg))
    console.log('Avg. runtime (s): ' + timeSolCtAvg.toFixed(3))
    console.log('Avg. solution length: ' + solSolCtAvg.toFixed(1))
...
function runAgent() {
    if (executingAgent) {
        return;
    }	//check if already running agent
