...
function overlapped(a, b) {
    return equals(a, b) || (a.x == b.x && a.y == b.y);
}
...
function drowned(phys, sinkers) {
    ...
    for (var p = 0; p < phys.length; p++) {
        for (var s = 0; s < sinkers.length; s++) {
            if ((!equals(phys[p], sinkers[s])) && (overlapped(phys[p], sinkers[s])))
...
function transformation(om, bm, rules, sort_phys, phys) {
    ...
    for (var r = 0; r < rules.length; r++) {
        ...
        if (equals(parts[0], parts[2]) && (parts[0] in sort_phys))
            xisx.push(parts[0]);
    }
...
function changeSprite(o, w, om, bm, phys, sort_phys) {
    ...
    if (equals(objAtPos(o.x, o.y, om), o))
        om[o.y][o.x] = o2;
    else
        bm[o.y][o.x] = o2;
...
function movePlayers(dir, mo, state) {
    ...
    // res = state['players']
    // res = destroyObjs(drowned(phys, sinkers), state);
    // res = state['players']
    //  res = drowned(phys, sinkers)
    // res = state
